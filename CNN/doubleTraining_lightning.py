...
class LitCNN(L.LightningModule):
    def __init__(self, weight_decay, input_channels=1, optimizer_name='adam', lr=1e-3):
        super().__init__()
...
def run_experiments(
    ...
    for run in range(num_runs):
        ...
        model_mnist = LitCNN(weight_decay=weight_decay, input_channels=1, optimizer_name='adam', lr=1e-3)  # MNIST: input_channels=1
        ...
        trainer.fit(model_mnist, mnist_dm)
        trainer.test(model_mnist, mnist_dm)

        # CIFAR10 training
        cifar10_dm = CIFAR10DataModule(batch_size=batch_size)
        model_cifar = LitCNN(weight_decay=weight_decay, input_channels=3, optimizer_name='sgd', lr=1e-2)  # CIFAR10: input_channels=3
        trainer.fit(model_cifar, cifar10_dm)
        trainer.test(model_cifar, cifar10_dm)
