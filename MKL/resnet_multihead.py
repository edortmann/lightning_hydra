...
class MKLModule:
    def __init__(self, weights, p=1., reg_rate=0.001):
        self.weights = weights
        self.p = p
        self.q = 2. if p == float('inf') else 2 * p / (p + 1)
        self.reg_rate = reg_rate

    def penalty(self):
        # Compute penalty = reg_rate * (sum(norms^q))^(2/q)
        norms = torch.stack([torch.linalg.matrix_norm(w) for w in self.weights])
        return self.reg_rate * (torch.sum(norms ** self.q)) ** (2 / self.q)

    def relevance_scores(self):
        betas = self.betas()
        return betas / torch.sum(betas)

    def betas(self):
        norms = torch.stack([torch.linalg.matrix_norm(w) for w in self.weights])
        a = norms ** (2 / (self.p + 1))
        b = torch.sum(norms ** self.q) ** (1 / self.p)
        return a / b


# -----------------------------------------------------------------------------
# Multihead Model with FCFusion and MKL Integration
# -----------------------------------------------------------------------------
...
class LitMultiHeadClassifier(L.LightningModule):
    def __init__(self, model, criterion, learning_rate, momentum, mkl_module=None):
        super().__init__()
        self.model = model
        self.criterion = criterion
        self.learning_rate = learning_rate
        self.momentum = momentum
        self.mkl_module = mkl_module
        ...
        self.val_accuracies = []
        self.epoch_relevance = [] if mkl_module is not None else None
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.mkl_module is not None:
            loss = loss + self.mkl_module.penalty()
    ...
    def on_train_epoch_end(self):
        ...
        if self.mkl_module is not None:
            self.model.eval()
            with torch.no_grad():
                rel_scores = self.mkl_module.relevance_scores().detach().cpu().numpy()
            self.epoch_relevance.append(rel_scores)
            self.model.train()
...
def plot_results_combined(epochs, train_losses, test_losses, train_accuracies, test_accuracies,
                          sum_head_weight_norms, standard_weight_norms, num_heads, output_dir, timestamp, relevance_history=None):
    ...
    if relevance_history is not None and len(relevance_history) > 0:
        relevance_history = np.array(relevance_history)  # shape: (epochs, num_heads)
        fig3, ax3 = plt.subplots(figsize=(8, 6))
        for i in range(relevance_history.shape[1]):
            ax3.plot(epochs[1:], relevance_history[:, i], marker='o', label=f"Head {i + 1}")
        ax3.set_xlabel("Epoch")
        ax3.set_ylabel("Relevance Score")
        ax3.set_title("Evolution of MKL Relevance Scores")
        ax3.legend()
        plt.tight_layout()
        plt.savefig(f"{output_dir}/relevance_scores_evolution_{timestamp}.png")
...
def main(cfg: DictConfig):
    ...
    output_dir = cfg.experiment.output_dir + f'/p{p}' + f'_reg{reg_rate}'
    ...
    # Initialize the multihead model using fusion (replacing the sum-of-heads model)
    multihead_model = CNNMultiLayerFusionModel(resnet18, backbone_feature_dims, hidden_dim, num_classes)

    # Standard model remains unchanged.
    ...
    # Create MKLModule using weights from the fusion layer (sub_weights)
    mkl_module = MKLModule(multihead_model.fusion.sub_weights, p=p, reg_rate=reg_rate)
    ...
    # Create Lightning trainers
    trainer1 = L.Trainer(
        max_epochs=epochs,
        accelerator='auto',
        default_root_dir=f"./lightning_logs/results_p{p}_reg{reg_rate}"
    )
    trainer2 = L.Trainer(
        max_epochs=epochs,
        accelerator='auto',
        default_root_dir=f"./lightning_logs/results_p{p}_reg{reg_rate}/standard_model"
    )
    ...
    plot_results_combined(
        ...
        output_dir,
        timestamp,
        relevance_history=lit_multihead.epoch_relevance
    )
